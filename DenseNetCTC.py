...
class USER():
    ...
    def build_model(self, summary=True):
        ...
        # DenseNet
        x = tf.keras.applications.densenet.preprocess_input(image)
        ...
        x = tf.keras.layers.Bidirectional(tf.keras.layers.GRU(512, return_sequences=True, implementation=2),
                                          name='blstm')(x)
        densenetoutput = tf.keras.layers.Dense(params_characters_len + 2, name='out', activation='softmax')(x)

        # CTC
        predict = CTCLayer(name="ctclayer")(inputs=(label, densenetoutput))
    ...
    def build_predict_model(self, summary=True):
        ...
        model = Model(inputs=[image], outputs=[densenetoutput])
